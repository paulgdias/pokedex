	...
	return (
		<>
			...
			<div className={`flex ${toggleWidth} transition-all duration-300`}>
				<nav className={`grid gap-4 bg-gray-800 p-2 w-full content-start`}>
					<Link
						href="/"
					...
					>
						...
						/>
						<span className={`text-white ml-2 ${toggleDisplay}`}>Home</span>
					</Link>
					<Link
						href="/pokedex"
						className={`react-aria-Button flex flex-row items-end ${hoverShadow}`}
					...
					>
						...
						/>
						<span className={`text-white ml-2 ${toggleDisplay}`}>Pokédex</span>
					</Link>
				</nav>
			</div>
